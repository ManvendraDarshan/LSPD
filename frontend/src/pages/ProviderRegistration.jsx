    ...
    <div className="page-shell">
      ...
      <form onSubmit={submit} className="grid gap-5 lg:grid-cols-2">
        ...
        <section className="card p-5 lg:col-span-2"><h2 className="font-bold">Business Profile</h2><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <label className="field"><span>Category</span><select required value={profile.category_ids[0] || ""} onChange={(e) => setProfile({ ...profile, category_ids: [e.target.value] })}><option value="">Select</option>{categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
        ...
        </div><button className="btn-primary mt-5">Submit provider profile</button></section>
      </form>
    </div>
  );
}
