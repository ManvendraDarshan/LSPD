...
export default function ProviderRegistration() {
  ...
  const [profile, setProfile] = useState({ business_name: "", description: "", experience: 1, address: "", city: "", district: "", state: "Madhya Pradesh", pincode: "", latitude: 23.2599, longitude: 77.4126, working_hours: "Mon-Sat, 9:00 AM - 7:00 PM", category_ids: [] });
  useEffect(() => {
    api.get("/categories")
      .then((res) => {
        setCategories(res.data);
        setCategoryError(res.data.length ? "" : "No active service categories are available.");
      })
      .catch((error) => setCategoryError(`Unable to load categories: ${errorMessage(error)}`))
  ...
  async function submit(e) {
    ...
    try {
      ...
      const payload = { ...profile, category_ids: profile.category_ids.map(Number) };
    ...
    <div className="page-shell">
      ...
      <form onSubmit={submit} className="grid gap-5 lg:grid-cols-2">
        ...
        <section className="card p-5 lg:col-span-2"><h2 className="font-bold">Business Profile</h2><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <label className="field"><span>Category</span><select required disabled={categoriesLoading || Boolean(categoryError)} value={profile.category_ids[0] || ""} onChange={(e) => setProfile({ ...profile, category_ids: e.target.value ? [e.target.value] : [] })}><option value="">{categoriesLoading ? "Loading categories..." : "Select a category"}</option>{categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select>{categoryError && <span className="text-sm text-red-600">{categoryError} Check that the API and database are running, then refresh.</span>}</label>
        ...
        </div><button className="btn-primary mt-5" disabled={categoriesLoading || Boolean(categoryError) || !profile.category_ids.length}>Submit provider profile</button></section>
      </form>
    </div>
  );
}
